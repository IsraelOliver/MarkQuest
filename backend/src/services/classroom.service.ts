...
export class ClassroomService {
  create(input: { unitId: string; name: string; year: string }): Classroom {
    ...
    if (!unit) {
      throw new AppError('UNIT_NOT_FOUND', 'Unidade informada nao existe.', 404)
    }
  ...
  delete(id: string) {
    ...
    if (hasExams) {
      throw new AppError('CLASSROOM_HAS_EXAMS', 'Nao e possivel excluir a turma enquanto houver provas vinculadas.', 400)
    }

    const hasStudents = db.students.some((item) => item.classroomId === id)
    if (hasStudents) {
      throw new AppError('CLASSROOM_HAS_STUDENTS', 'Nao e possivel excluir a turma enquanto houver alunos cadastrados.', 400)
    }
